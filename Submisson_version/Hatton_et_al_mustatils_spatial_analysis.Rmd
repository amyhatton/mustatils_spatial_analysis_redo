...
Run the "DEM_derivitives.R" script to calculate a bunch of landform classifications
Warning - This will take a while to run!
```{r, include=FALSE}
sys.source("DEM_derivitives.R", envir = knitr::knit_global())

tpi_stack <- raster::brick("data/tpi_stack.tif")
tri_stack <- raster::brick("data/tri_stack.tif")
roughness_stack <- raster::brick("data/roughness_stack.tif")
...
```{r}
#read in vector data
rastlist <- list.files(path = "data", pattern='.tif$', all.files= T, full.names= T)
rastlist <- rastlist[-2]
rasts <- lapply(rastlist, rast)
names(rasts)
rasts$aspect
rasters <- terra::rast(rastlist)
rasts[[1]]
rastlist[2]
mustatils <- st_read("data/mustatils.gpkg")
study_area <- st_read("data/study_area.gpkg")
dem <- raster("data/dem.tif")
dist_lakes <- raster("data/dist_lakes.tif")
dist_rivers <- raster("data/dist_rivers.tif")
dist_deposits <- raster("data/dist_deposits.tif")
dist_lakes_rivers <- raster("data/dist_lakes_rivers.tif")
dist_deposits_rivers <- raster("data/dist_deposits_rivers.tif")
dist_sandstone_geo <- raster("data/dist_sandstone_geo.tif")
dist_non_quart_geo <- raster("data/dist_non_quart_geo.tif")
viewshed <- raster("data/t_vshed_whole_scaled.tif")
dist_pekel_water <- raster("data/dist_pekel_mask.tif")
...
Run the other script to create some other layers derived from DEM
```{r}
...
variables that have predictive power - will check for collinearity afterwards using VIF
- slope,
- tpi_stack$tpi_3 
- tpi_stack$tpi_5, 
- tri_stack$tri_3, 
