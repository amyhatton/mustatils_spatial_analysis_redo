...
pekel_water <- raster("data/projected_utm37n/pekel_mask.tif")
```
...
r = list(slope,aspect,tpi_stack$tpi_3, tpi_stack$tpi_5, tpi_stack$tpi_15,
         ...
         dist_sandstone_geo,dist_non_quart_geo,dist_lakes, viewshed, northness, eastness)
...
#rename to make sure they're right
...
locvals_all_sites <- data.frame(terra::extract(covariates, all_sites_v))
...
par(mfrow=c(2,1))
hist(locvals_all_sites$slope[locvals_all_sites$site==1], breaks=seq(0,45,5))
hist(locvals_all_sites$slope[locvals_all_sites$site==0], breaks=seq(0,45,5))
par(mfrow=c(1,1))
range(locvals_all_sites$slope)
#tpi3- looks good skewed to more  positive values for mustatils
par(mfrow=c(2,1))
hist(locvals_all_sites$tpi_3[locvals_all_sites$site==1], breaks=seq(-10,10,1))
hist(locvals_all_sites$tpi_3[locvals_all_sites$site==0], breaks=seq(-10,10,1))
par(mfrow=c(1,1))

#tpi5- looks good skewed to more  negative values for mustatils
par(mfrow=c(2,1))
hist(locvals_all_sites$tpi_5[locvals_all_sites$site==1], breaks=seq(-50,50,10))
hist(locvals_all_sites$tpi_5[locvals_all_sites$site==0], breaks=seq(-50,50,10))
par(mfrow=c(1,1))

#tpi15- not great predictor, skewed slighty negative for mustatils
par(mfrow=c(2,1))
hist(locvals_all_sites$tpi_15[locvals_all_sites$site==1], breaks=seq(-80,110,10))
hist(locvals_all_sites$tpi_15[locvals_all_sites$site==0], breaks=seq(-80,110,10))
par(mfrow=c(1,1))

#tri3- mustatils skewed to higher values - not much of a differnect though
par(mfrow=c(2,1))
hist(locvals_all_sites$tri_3[locvals_all_sites$site==1], breaks=seq(0,20,1))
hist(locvals_all_sites$tri_3[locvals_all_sites$site==0], breaks=seq(0,20,1))
par(mfrow=c(1,1))

#tri5- mustatils skewed to hiher values - not much of a differnect though
par(mfrow=c(2,1))
hist(locvals_all_sites$tri_5[locvals_all_sites$site==1], breaks=seq(0,150,5))
hist(locvals_all_sites$tri_5[locvals_all_sites$site==0], breaks=seq(0,150,5))
par(mfrow=c(1,1))
range(locvals_all_sites$tri_15)
...
par(mfrow=c(2,1))
hist(locvals_all_sites$rough_3[locvals_all_sites$site==1], breaks=seq(0,55,5))
hist(locvals_all_sites$rough_3[locvals_all_sites$site==0], breaks=seq(0,55,5))
par(mfrow=c(1,1))

#roughness5- mustatils skewed to hiher values - not much of a differnect though
par(mfrow=c(2,1))
hist(locvals_all_sites$rough_5[locvals_all_sites$site==1], breaks=seq(0,105,5))
hist(locvals_all_sites$rough_5[locvals_all_sites$site==0], breaks=seq(0,105,5))
par(mfrow=c(1,1))
...
par(mfrow=c(2,1))
hist(locvals_all_sites$dist_lakes_rivers [locvals_all_sites$site==1], breaks=seq(0,20000,1000))
hist(locvals_all_sites$dist_lakes_rivers [locvals_all_sites$site==0], breaks=seq(0,20000,1000))
par(mfrow=c(1,1))

#dist deposits rivers similar to lakes rivers also looks good
par(mfrow=c(2,1))
hist(locvals_all_sites$dist_deposits_rivers[locvals_all_sites$site==1], breaks=seq(0,20000,1000))
hist(locvals_all_sites$dist_deposits_rivers[locvals_all_sites$site==0], breaks=seq(0,20000,1000))
par(mfrow=c(1,1))

#dist sandstone geo mustatils mostly clustered in first km
par(mfrow=c(2,1))
hist(locvals_all_sites$dist_sandstone_geo[locvals_all_sites$site==1], breaks=seq(0,53000,1000))
hist(locvals_all_sites$dist_sandstone_geo[locvals_all_sites$site==0], breaks=seq(0,53000,1000))
par(mfrow=c(1,1))
...
summary(slope.m)
lrtest(slope.m) #very good lrt pvalue of 2.692e-06 ***
...
summary(northness.m)
lrtest(northness.m) #bad p value 0.5826

#tpi
tpi3.m <- glm(site~tpi_3, data = locvals_all_sites, family=binomial(logit))
summary(tpi.m)
lrtest(tpi.m) #very good pvalue 2.2e-16 ***
...
summary(tpi15.m)
lrtest(tpi15.m) #okay p-value 0.01955 *

#tri
tri3.m <- glm(site~tri_3, data = locvals_all_sites, family=binomial(logit))
summary(tri3.m)
lrtest(tri3.m) #very good pvalue 1.564e-07 ***
...
summary(dem.m)
lrtest(dem.m) # good pvalue 2.628e-06 ***
...
removeCollinearity(covariates, multicollinearity.cutoff = 0.7,
...
names(im_stack) <- c("tpi_3_im", "tpi_5_im", 
        "rough_5_im","dem_im","dist_rivers_im",
        "dist_sandstone_geo_im","viewshed_im","eastness_im")
im_stack

c_trend <- ~tpi_3_im + tpi_5_im + rough_5_im +dem_im + dist_rivers_im + 
  dist_sandstone_geo_im + viewshed_im + eastness_im 
```
...
pdf(width=5, height=4, file = "plots/pcf_final/pcf_null_final_999sim.pdf")
...
pdf(width=9, height=7, file = "plots/pcf_first_order_zoomed_999sim.pdf")
...
plot(mod1_surf)
pdf(width=9, height=7, file = "plots/predicted_intensity_1order_test_1.pdf")
...
pdf(width=9, height=7, file = "plots/predicted_intensity_1order_test_2.pdf")
...
summary(mod_2nd_o)
pdf(width=9, height=7, file = "plots/pcf_2nd_order_zoomed_999sim.pdf")
...
```{r plot pcf}
pdf(width=10, height=3, file = "plots/pcfs.pdf")
par(mfrow=c(1,3))
